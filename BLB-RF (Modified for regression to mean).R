...
points_no_se <- function(train, test, output, cluster = FALSE, hist = FALSE) {
  ...
  yexp <- c(output[,3],output[,3])
  yhat <- c(output[,1], ij[,1])
  se <- c(output[,2], ij[,2])
  
  # Construct the data frame for plotting
  df <- data.frame(yexp, yhat, se, indicator = c(rep('BLB',nrow(test)), rep('RF', nrow(test))), residual = (yhat - yexp))
  ...
  if(!cluster && !hist){
    #print(11)
      ...
      xlab("Truth: y = Bx") + 
      ylab("Prediction") +
      ggtitle("Random Forest vs Random Forest-BLB")  + scale_colour_discrete(name  ="Legend", labels=c("BLB", "RF"))
